 ...
 * as a feature in the segmenter, and
 * 
...
public class ChineseDictionary {
	...
	private static HashMap<String,ChineseDictionary> instanceslib=new HashMap<String,ChineseDictionary>();
	
	
	public static ChineseDictionary getInstanceByDicts(String[] dicts,ChineseDocumentToSentenceProcessor cdtos,boolean expandMidDot)
	{
		
		String key=dicts2key(dicts);
		
		if(!(instanceslib.containsKey(key)))
		{
			instanceslib.put(key, new ChineseDictionary(dicts,cdtos,expandMidDot));
		}
	
		return instanceslib.get(key);
		
	}
	
	public static String dicts2key(String[] dicts)
	{
		String key="";
		
		for(int i=0;i<dicts.length;i++)
		{
			key=key+dicts[i].trim();
		}
	...
	private void serializeDictionary(String serializePath) {
		System.err
				.print("Serializing dictionaries to " + serializePath + "...");
	...
	private static Set<String>[] loadDictionary(String serializePath) {
		...
		try {
			...
			dict = IOUtils
					.readObjectFromURLOrClasspathOrFileSystem(serializePath);
		} catch (Exception e) {
			System.err.println("Failed to load Chinese dictionary "
					+ serializePath);
			throw new RuntimeException(e);
		}
		return dict;
	...
	public ChineseDictionary(String serDicts,
			ChineseDocumentToSentenceProcessor cdtos, boolean expandMidDot) {
		this(serDicts.split(","), cdtos, expandMidDot);
	}

	public ChineseDictionary(String[] dicts,
			ChineseDocumentToSentenceProcessor cdtos, boolean expandMidDot) {
		System.err.println("Loading Chinese dictionaries from " + dicts.length
				+ " files:");
		...
		System.err.println("Done. Unique words in ChineseDictionary is: "
				+ total);
	}

	private final Pattern midDot = Pattern
	...
	private void addDict(String dict, boolean expandMidDot) {
		...
		try {
			InputStream fis = IOUtils
					.getInputStreamFromURLOrClasspathOrFileSystem(dict);
		...
		for (String line : lines) {
			...
			if (expandMidDot) {
				...
				line = line
						.replaceAll(ChineseUtils.MID_DOT_REGEX_STR, "\u00B7");
			}
			addOneDict(line);
			if (DEBUG)
	...
	public static void main(String[] args) {
		...
		ChineseDocumentToSentenceProcessor cdtos = new ChineseDocumentToSentenceProcessor(
				null);
		boolean expandMidDot = true;

		ChineseDictionary dict = new ChineseDictionary(dicts, cdtos,
				expandMidDot);
		dict.serializeDictionary(output);

		/*
		 * //ChineseDictionary dict = new ChineseDictionary(args[0]); for (int i
		 * = 0; i <= MAX_LEXICON_LENGTH; i++) { System.err.println("Length: " +
		 * i+": "+dict.words[i].size()); } for (int i = 0; i <=
		 * MAX_LEXICON_LENGTH; i++) { System.err.println("Length: " +
	...
	public HashMap<String,ChineseDictionary> getInstanceslib() {
		return instanceslib;
	}

	public void setInstanceslib(HashMap<String,ChineseDictionary> instanceslib) {
		this.instanceslib = instanceslib;
	}
