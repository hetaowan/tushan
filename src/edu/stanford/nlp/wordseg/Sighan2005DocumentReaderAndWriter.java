...
public class Sighan2005DocumentReaderAndWriter implements DocumentReaderAndWriter<CoreLabel>, LatticeWriter<CoreLabel> {
  ...
  private static Logger logger = Logger.getLogger(Sighan2005DocumentReaderAndWriter.class);

  public Iterator<List<CoreLabel>> getIterator(Reader r) {
	//System.out.println("factory:"+factory.getClass().getCanonicalName());
  ...
  public void init(SeqClassifierFlags flags) {
    ...
    if (flags.dictionary != null) {
      ...
      ///dict////cdict = new ChineseDictionary(dicts, cdtos, flags.expandMidDot);
      cdict =ChineseDictionary.getInstanceByDicts(dicts, cdtos, flags.expandMidDot);
    }
    if (flags.serializedDictionary != null) {//从makefile -serDictionary $(SEGMENTER)/data/dict-chris6.ser.gz 获得值
      String dict = flags.serializedDictionary;
    ///dict////cdict = new ChineseDictionary(dict, cdtos, flags.expandMidDot);
      String[] tempDicts={dict};
      cdict=ChineseDictionary.getInstanceByDicts(tempDicts,cdtos, flags.expandMidDot);
    }

    if (flags.dictionary2 != null) {//为什么多个词典?
      String[] dicts2 = flags.dictionary2.split(",");
      //System.out.println("flags.dictionary2:"+flags.dictionary2);
    ///dict////cdict2 = new ChineseDictionary(dicts2, cdtos, flags.expandMidDot);
      cdict2=ChineseDictionary.getInstanceByDicts(dicts2, cdtos, flags.expandMidDot);
    }
  }
  ...
  public class CTBDocumentParser implements Function<String,List<CoreLabel>> {
    ...
    public List<CoreLabel> apply(String line) {//对于一个line 返回CoreLabel的列表，每个元素wi 设置各种属性，例如   wi.set(AnswerAnnotation.class, "1") 或  wi.set(UBlockAnnotation.class, Characters.unicodeBlockStringOf(ch))
      if (line == null) {
        return null;
      } 
      //System.out.println("in Sighan apply");
      ...
      logger.debug("apply origLine :"+origLine);
      logger.debug("apply line:"+line);
      ...
      for (int index = 0, len = line.length(); index < len; index++) {
        ...
        if ( ! Character.isWhitespace(ch) && ! Character.isISOControl(ch)) {//跳过空格回车换行，对于空格回车换行等不生成相应的wi
      ...
      {
    	  logger.debug("apply lwi.size:"+lwi.size());
          for(int li=0;li<lwi.size();li++)
          {
        	  CoreLabel cl=lwi.get(li);
        	  logger.debug("apply cl2:"+cl.toString());
          }
      }
