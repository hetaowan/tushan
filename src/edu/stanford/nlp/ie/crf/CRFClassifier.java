...
import edu.stanford.common.CRFFeatureSelection;
import edu.stanford.common.ClassifierCommon;
import edu.stanford.common.Coordinate;
 ...
 * class. The default for <code>ColumnDocumentReaderAndWriter</code> training
 * data is 3 column input, with the columns containing a word, its POS, and its
 * gold class, but this can be specified via the <code>map</code> property. </p>
 * <p>
 * When run on a file with <code>-textFile</code>, the file is assumed to be
 * plain English text (or perhaps simple HTML/XML), and a reasonable attempt is
 * made at English tokenization by {@link PlainTextDocumentReaderAndWriter}. The
 * class used to read the text can be changed with
 * -plainTextDocumentReaderAndWriter. Extra options can be supplied to the
 * tokenizer using the -tokenizeOptions flag.
 * </p>
 * <p>
 ...
 */
public class CRFClassifier<IN extends CoreMap> extends
		AbstractSequenceClassifier<IN> {
	...
	private Map<String, List<Coordinate>> featureCoordinates = null;
	private int[] activeMap;
	private int[] selectMap;
	private List<int[]> labels;
	private int docIndex = 0;
	private boolean isInit = false;
	private double[][] Ehat = null;
	private CRFCliqueTree[] cliqueTrees = null;
	private boolean inEmpirial = false;
	// ////////////////////////////////
	/**
	...
	private static Logger logger = Logger.getLogger(CRFClassifier.class);
	...
	public CRFClassifier(CRFClassifier<IN> crf) {
		...
		this.pad = crf.pad;
		this.knownLCWords = (crf.knownLCWords != null) ? new HashSet<String>(
				crf.knownLCWords) : null;
		this.featureIndex = (crf.featureIndex != null) ? new HashIndex<String>(
				crf.featureIndex.objectsList()) : null;
		if (crf.flags.nonLinearCRF) {
			this.nodeFeatureIndicesMap = (crf.nodeFeatureIndicesMap != null) ? new HashIndex<Integer>(
					crf.nodeFeatureIndicesMap.objectsList()) : null;
			this.edgeFeatureIndicesMap = (crf.edgeFeatureIndicesMap != null) ? new HashIndex<Integer>(
					crf.edgeFeatureIndicesMap.objectsList()) : null;
		}
		this.classIndex = (crf.classIndex != null) ? new HashIndex<String>(
				crf.classIndex.objectsList()) : null;
		if (crf.labelIndices != null) {
			this.labelIndices = new HashIndex[crf.labelIndices.length];
			for (int i = 0; i < crf.labelIndices.length; i++) {
				this.labelIndices[i] = (crf.labelIndices[i] != null) ? new HashIndex<CRFLabel>(
						crf.labelIndices[i].objectsList()) : null;
			}
		} else {
		...
		for (int i = 0; i < numFeatures; i++) {
			...
			System.arraycopy(crf.weights[index], 0, weights[i], 0,
					weights[i].length);
		}
	}
	...
	private void combineWeights(CRFClassifier<IN> crf, double weight) {
		...
		for (int i = 0; i < crf.labelIndices.length; i++) {
			for (int j = 0; j < crf.labelIndices[i].size(); j++) {
				...
				for (int ci = 0; ci <= i; ci++) {
					String classLabel = crf.classIndex
							.get(labels.getLabel()[ci]);
				...
				int k = this.labelIndices[i].indexOf(newLabels); // the indexing
																	// is
																	// needed,
																	// even when
																	// not
																	// printed
																	// out!
				// System.err.println("LabelIndices " + i + " " + labels + ": "
				// + j +
				// " mapped to " + k);
			}
		}
		...
		for (int i = 0; i < numFeatures; i++) {
			...
			if (i < oldNumFeatures) {
				...
				System.arraycopy(weights[i], 0, newWeights[i], 0,
						weights[i].length);
			}
		}
		weights = newWeights;
		...
		for (int i = 0; i < crf.weights.length; i++) {
			...
			if (weights[newIndex].length < crf.weights[i].length) {
				throw new RuntimeException(
						"Incompatible CRFClassifier: weight length mismatch for feature "
								+ newIndex + ": " + featureIndex.get(newIndex)
								+ " (also feature " + i + ": "
								+ crf.featureIndex.get(i) + ") " + ", len1="
								+ weights[newIndex].length + ", len2="
								+ crf.weights[i].length);
			}
	...
	public void combine(CRFClassifier<IN> crf, double weight) {
		...
		System.err
				.println("Combining weights: will automatically match labelIndices");
		combineWeights(crf, weight);

		int numFeatures = featureIndex.size();
		int numWeights = getNumWeights();
		long elapsedMs = timer.stop();
		System.err.println("numFeatures: orig1=" + oldNumFeatures1 + ", orig2="
				+ oldNumFeatures2 + ", combined=" + numFeatures);
		System.err.println("numWeights: orig1=" + oldNumWeights1 + ", orig2="
				+ oldNumWeights2 + ", combined=" + numWeights);
		System.err.println("Time to combine CRFClassifier: "
		...
		try {
			...
			for (int i = 0; i < weights.length; i++) {
				...
				for (int j = 1; j < weights[i].length; j++) {
					...
					logger.debug("mehtod dropFeaturesBelowThreshold " + i + " "
							+ featureIndex.get(i) + " " + biggest + " "
							+ smallest);
					...
					if (biggest - smallest > threshold) {
						...
						logger.debug("mehtod dropFeaturesBelowThreshold remove:"
								+ i
								+ " "
								+ featureIndex.get(i)
								+ " "
	...
	public Pair<int[][][], int[]> documentToDataAndLabels(List<IN> document,
		...
		// third index is position of the feature in the array that holds
		// them.第三个索引文档指定位置指定窗口大小不同的特征
		// An element in data[j][k][m] is the index of the mth feature occurring
		// in
		// position k of the jth clique
		// System.out.println("windowsize_here:"+windowSize);
		int[][][] data = new int[docSize][windowSize][];
		// index is the position in the document.
		// element in labels[j] is the index of the correct label (if it exists)
		// at
		...
		// System.out.println("featureFactory:"+featureFactory.toString());
		// System.err.println("docSize:"+docSize);
		
		/*
		for(int j=0;j<document.size();j++)
		{
			logger.info("jj="+j+"  "+document.get(j).toString());
		}
        */
		...
		for (int j = 0; j < docSize; j++) {
			...
			for (int k = 0, fSize = features.size(); k < fSize; k++) {
				...
				for (String feature : cliqueFeatures) {
					...
					if(flags.isInnerFeatureSelection==true)
					{
					 if (featureCoordinates != null) {
						if (!(featureCoordinates.containsKey(feature))) {
							continue;
						}
						if (isInit == false && inEmpirial == false) {
							// logger.info("feature:"+feature+" docIndex:"+docIndex+" j:"+j);
							featureCoordinates.get(feature).add(
									new Coordinate(docIndex, j));
						}
					 ...
					 * flags.inputDropOut) { if (!droppedFeature) {
					 * System.err.printf("Dropped feature %s%n", feature);
					 * droppedFeature = true; } continue; } }
					 */// ////
					...
					if(flags.isInnerFeatureSelection==true)
					{
					if (activeMap != null && inEmpirial == false) {
					...
					// System.out.println("feature:"+feature+"   index"+index);
					if (index >= 0) {
						data[j][k][m] = index;
						m++;
					} else {
						// this is where we end up when we do feature threshold
						// cutoffs
					}
				}
			...
			logger.debug("method documentToDataAndLabels wi:" + wi.toString());
		...
		for (int i = 0; i < docData.length; i++) {
			...
			for (int j = 0; j < docData[i].length; j++) {
				...
				for (int n = 0; n < cliqueFeatures.length; n++) {
					...
					if (j == 0) {
						transFeatureIndex = nodeFeatureIndicesMap
								.indexOf(cliqueFeatures[n]);
					} else {
						transFeatureIndex = edgeFeatureIndicesMap
								.indexOf(cliqueFeatures[n]);
					}
	...
	public void printLabelInformation(String testFile,
			DocumentReaderAndWriter<IN> readerAndWriter) throws Exception {
	...
	public void printLabelValue(List<IN> document) {
		...
		for (int j = 0; j < document.size(); j++) {
			...
			System.out.println("--== "
					+ document.get(j).get(TextAnnotation.class) + " ==--");
			...
			for (int p = 0; p < labelIndices.length; p++) {
				...
				CRFDatum<List<String>, CRFLabel> d = makeDatum(document, j + p,
						featureFactory);
	 ...
	 */
	public List<Pair<int[][][], int[]>> documentsToDataAndLabelsList(
			Collection<List<IN>> documents) {
		int numDatums = 0;
	...
	protected void printFeatures() {
		...
		try {
			...
			PrintWriter pw = new PrintWriter(new OutputStreamWriter(
					new FileOutputStream("features-" + flags.printFeatures
							+ ".txt"), enc), true);
	...
	protected void makeAnswerArraysAndTagIndex(Collection<List<IN>> ob) {
		...
		logger.debug("windowSize:" + "ob.size:" + ob.size());

		HashSet<String>[] featureIndices = new HashSet[windowSize];// 不同窗口大小特征的索引
		for (int i = 0; i < windowSize; i++) {
			featureIndices[i] = new HashSet<String>();
		}

		labelIndices = new HashIndex[windowSize];// 不同窗口大小标注的索引例如:
													// lableIndices[0]={[0],[1]}
													// labelIndices[1]={[0 0],[0
													// 1],[1 0],[1 1]}
		for (int i = 0; i < labelIndices.length; i++) {
		...
		classIndex = new HashIndex<String>();// 存储不同的标注
		// classIndex.add("O");
		...
		for (List<IN> doc : ob) {
			...
			for (int j = 0; j < docSize; j++) {
				...
				if (ans == null || ans.equals("")) {
					throw new IllegalArgumentException("Word " + wordCount
							+ " (\"" + doc.get(j).get(TextAnnotation.class)
							+ "\") has a blank answer");
				}
			...
			for (int j = 0; j < docSize; j++) {
				...
				CRFDatum<List<String>, CRFLabel> d = makeDatum(doc, j,
						featureFactory);
				...
				for (int k = 0, fsize = features.size(); k < fsize; k++) {
					...
					if (ClassifierCommon.verbosity_level >= ClassifierCommon.DEBUG) {
						for (String f : cliqueFeatures) {
							logger.debug("method makeAnswerArraysAndTagIndex feature:"
									+ f);
						}
					}
					...
					if (k < 2 && flags.removeBackgroundSingletonFeatures) {
						...
						// pw.println("background:"+background);
						...
						if (background) {
							for (String f : cliqueFeatures) {
								...
								logger.debug("method makeAnswerArraysAndTagIndex feature back:"
										+ f);
		...
		if (ClassifierCommon.verbosity_level == ClassifierCommon.DEBUG) {
			for (int li = 0; li < labelIndex.size(); li++) {
				CRFLabel cl = labelIndex.get(li);
				logger.debug(li + "::" + cl.toString());
			}
		}
		...
		featureIndex = new HashIndex<String>();// featureIndex存储所有的特征
		map = new int[numFeatures];// map的大小等于所有特征的个数
		for (int i = 0; i < windowSize; i++) {
			...
			for (String str : featureIndices[i]) {
				...
				map[index] = i;
				logger.debug("feature:" + str + " map:" + i);
				featureIndexMap.add(index);
			}
			if (i == 0) {
				nodeFeatureIndicesMap = featureIndexMap;// 用来
														// 判断某个特征是node特征还是edge特征，输入是特征索引，方法是查看nodeFeatureIndicesMap或edgeFeatureIndicesMap里有没有该索引
			} else {
	...
	public CRFDatum<List<String>, CRFLabel> makeDatum(List<IN> info, int loc,
			edu.stanford.nlp.sequences.FeatureFactory<IN> featureFactory) {//该方法待阅读
		// pad.set(AnswerAnnotation.class, flags.backgroundSymbol); // cdm:
		// isn't this unnecessary, as this is how it's initialized in
		// AbstractSequenceClassifier.reinit?

		// logger.info("in CRFCLassifier makeDatum info:" + info.size());
		// logger.info("in CRFCLassifier makeDatum pad.size:" + pad.size());
		//////为什么在这里附加,而不再documentsTozDataAndLabels
		/////[[[若i超越了界限，则返回padding]]]
		PaddedList<IN> pInfo = new PaddedList<IN>(info, pad);// pad
																// 为附加的CoreLabel
																// pad.set(AnswerAnnotation.class,
																// flags.backgroundSymbol)
																// pad.set(GoldAnswerAnnotation.class,
																// flags.backgroundSymbol)
		// logger.info("in CRFCLassifier makeDatum pInfo.size:" + pInfo.size());
		
		/*
		for(int j=0;j<pInfo.size();j++)
		{
			logger.info("jjj="+pInfo.get(j).toString());
		}
		*/
		...
		// todo [cdm Aug 2012]: Since getCliques returns all cliques within its
		// bounds, can't the for loop here be eliminated? But my first attempt
		// to removed failed to produce identical results....
		...
		for (int i = 0; i < windowSize; i++) {
			...
			logger.info("i="+i+" windowCliques.size:"+windowCliques.size());
			for (Clique c : windowCliques) {
				 logger.info("clique c1:"+c.toString());
			}
			
			windowCliques.removeAll(done);
			done.addAll(windowCliques);
			for (Clique c : windowCliques) {
				logger.info(i+": clique c:"+c.toString());
				featuresC.addAll(featureFactory
						.getCliqueFeatures(pInfo, loc, c)); // todo useless copy
															// because of typing
															// reasons
			}
			// [This is the version that didn't work.]
			// List<Clique> windowCliques = FeatureFactory.getCliques(windowSize
			// - 1, 0); // -1 as <= in getCliques()
			// List<List<String>> features = new
			// ArrayList<List<String>>(windowCliques.size());
			// for (Clique c : windowCliques) {
			// List<String> featuresC = new
			// ArrayList<String>(featureFactory.getCliqueFeatures(pInfo, loc,
			// c)); //todo useless copy because of typing reasons
			features.add(featuresC);
		}
		...
		CRFDatum<List<String>, CRFLabel> d = new CRFDatum<List<String>, CRFLabel>(
				features, new CRFLabel(labels));

		//logger.debug("method makeDatum crfdatum:" + d.toString());
	...
	public static class TestSequenceModel implements SequenceModel {
		 ...
		 * 
		 * @param tags
		 *            is an array indicating the assignment of labels to score.
		 * @param pos
	...
	public List<IN> classify(List<IN> document) {
		if (flags.doGibbs) {
			try {
				//System.out.println("classify gibbs");
			...
			//System.out.println("classify max ent");
	...
	private List<IN> classify(List<IN> document,
	...
	public void classifyAndWriteAnswers(Collection<List<IN>> documents,
			...
			PrintWriter printWriter, DocumentReaderAndWriter<IN> readerAndWriter)
			throws IOException {
		Timing timer = new Timing();
	...
	public SequenceModel getSequenceModel(List<IN> doc) {
		Pair<int[][][], int[]> p = documentToDataAndLabels(doc);//getFeatures
	...
	private CliquePotentialFunction getCliquePotentialFunction() {
		if (flags.nonLinearCRF) {
			if (flags.secondOrderNonLinear)
				return new NonLinearSecondOrderCliquePotentialFunction(
						inputLayerWeights4Edge, outputLayerWeights4Edge,
						inputLayerWeights, outputLayerWeights, flags);
			else
	...
	private List<IN> classifyMaxEnt(List<IN> document,
			Pair<int[][][], int[]> documentDataAndLabels) {
		if (document.isEmpty()) {
			return document;
		}
	...
	public List<IN> classifyGibbs(List<IN> document,
		...
		if (flags.useNERPrior) {
			EntityCachingAbstractSequencePrior<IN> prior = new EmpiricalNERPrior<IN>(
					flags.backgroundSymbol, classIndex, newDocument);
			// SamplingNERPrior prior = new
			// SamplingNERPrior(flags.backgroundSymbol,
			// classIndex, newDocument);
			priorModel = prior;
			priorListener = prior;
		} else if (flags.useAcqPrior) {
			EntityCachingAbstractSequencePrior<IN> prior = new AcquisitionsPrior<IN>(
					flags.backgroundSymbol, classIndex, newDocument);
			priorModel = prior;
			priorListener = prior;
		} else if (flags.useSemPrior) {
			EntityCachingAbstractSequencePrior<IN> prior = new SeminarsPrior<IN>(
					flags.backgroundSymbol, classIndex, newDocument);
			priorModel = prior;
			priorListener = prior;
		} else if (flags.useUniformPrior) {
			// System.err.println("Using uniform prior!");
			UniformPrior<IN> uniPrior = new UniformPrior<IN>(
					flags.backgroundSymbol, classIndex, newDocument);
			priorModel = uniPrior;
		...
		if (flags.annealingType.equalsIgnoreCase("linear")) {
			sequence = sampler.findBestUsingAnnealing(model,
					CoolingSchedule.getLinearSchedule(1.0, flags.numSamples),
					sequence);
	 ...
	 * @param modelWts
	 *            an array of model weights: IMPORTANT: this includes the weight
	 *            of CRF classifier as well at position 0, and therefore is
	 *            longer than priorListeners/priorModels array by 1.
	...
	public List<IN> classifyGibbsUsingPrior(List<IN> sentence,
			SequenceModel[] priorModels, SequenceListener[] priorListeners,
			double[] modelWts) throws ClassNotFoundException,
	...
	public List<IN> classifyGibbsUsingPrior(List<IN> sentence,
			SequenceModel priorModel, SequenceListener priorListener,
			double model1Wt, double model2Wt) throws ClassNotFoundException,
			SecurityException, NoSuchMethodException, IllegalArgumentException,
			InstantiationException, IllegalAccessException,
			InvocationTargetException {
		// System.err.println("Testing using Gibbs sampling.");
	...
	public CRFCliqueTree<String> getCliqueTree(Pair<int[][][], int[]> p) {
		...
		return CRFCliqueTree.getCalibratedCliqueTree(data, labelIndices,
				classIndex.size(), classIndex, flags.backgroundSymbol,
	 ...
	 * 
	 * 输入： [CharAnnotation=中 OriginalCharAnnotation=中 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=0
	 * LBeginAnnotation=2 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=国 OriginalCharAnnotation=国 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=1
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=2]
	 * [CharAnnotation=十 OriginalCharAnnotation=十 ShapeAnnotation=N
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=2
	 * LBeginAnnotation=3 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=四 OriginalCharAnnotation=四 ShapeAnnotation=N
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=3
	 * LBeginAnnotation=2 LMiddleAnnotation=3 LEndAnnotation=2]
	 * [CharAnnotation=个 OriginalCharAnnotation=个 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=4
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=3]
	 * [CharAnnotation=边 OriginalCharAnnotation=边 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=5
	 * LBeginAnnotation=2 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=境 OriginalCharAnnotation=境 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=6
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=2]
	 * [CharAnnotation=开 OriginalCharAnnotation=开 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=7
	 * LBeginAnnotation=4 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=放 OriginalCharAnnotation=放 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=8
	 * LBeginAnnotation=1 LMiddleAnnotation=4 LEndAnnotation=2]
	 * [CharAnnotation=城 OriginalCharAnnotation=城 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=9
	 * LBeginAnnotation=2 LMiddleAnnotation=4 LEndAnnotation=1]
	 * [CharAnnotation=市 OriginalCharAnnotation=市 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=10
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=4]
	 * [CharAnnotation=经 OriginalCharAnnotation=经 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=11
	 * LBeginAnnotation=4 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=济 OriginalCharAnnotation=济 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=12
	 * LBeginAnnotation=1 LMiddleAnnotation=4 LEndAnnotation=2]
	 * [CharAnnotation=建 OriginalCharAnnotation=建 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=13
	 * LBeginAnnotation=2 LMiddleAnnotation=4 LEndAnnotation=1]
	 * [CharAnnotation=设 OriginalCharAnnotation=设 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=14
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=4]
	 * [CharAnnotation=成 OriginalCharAnnotation=成 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=15
	 * LBeginAnnotation=2 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=就 OriginalCharAnnotation=就 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=16
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=2]
	 * [CharAnnotation=显 OriginalCharAnnotation=显 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=1 SpaceBeforeAnnotation=1 PositionAnnotation=17
	 * LBeginAnnotation=2 LMiddleAnnotation=0 LEndAnnotation=1]
	 * [CharAnnotation=著 OriginalCharAnnotation=著 ShapeAnnotation=C
	 * UTypeAnnotation=5 UBlockAnnotation=CJK_UNIFIED_IDEOGRAPHS
	 * AnswerAnnotation=0 SpaceBeforeAnnotation=0 PositionAnnotation=18
	 * LBeginAnnotation=1 LMiddleAnnotation=0 LEndAnnotation=2]
	 * 
	 * @param docs
	 *            A Collection (perhaps ObjectBank) of documents
	 */
	@Override
	public void train(Collection<List<IN>> docs,
			DocumentReaderAndWriter<IN> readerAndWriter) {
		// logger.info("in crf classifier train");
		//System.out.println("in crf classifier train");
		Timing timer = new Timing();
		timer.start();
		makeAnswerArraysAndTagIndex(docs);
		long elapsedMs = timer.stop();
		System.err.println("Time to convert docs to feature indices: "
				+ Timing.toSecondsString(elapsedMs) + " seconds");
		if (flags.exportFeatures != null) {
			timer.start();
			CRFFeatureExporter<IN> featureExporter = new CRFFeatureExporter<IN>(
					this);
			featureExporter.printFeatures(flags.exportFeatures, docs);
			elapsedMs = timer.stop();
			System.err.println("Time to export features: "
					+ Timing.toSecondsString(elapsedMs) + " seconds");
		}
		...
		{
		inEmpirial = true;
		initEmpiricalCounts(docs);
		inEmpirial = false;

		// 初始化活跃特征map,初始时所有特征都不活跃
		activeMap = new int[featureIndex.size()];
		selectMap = new int[featureIndex.size()];
		featureCoordinates = new HashMap<String, List<Coordinate>>();

		for (String feature : featureIndex) {
			// logger.info("in train feature:"+feature);
			featureCoordinates.put(feature, new ArrayList<Coordinate>());
		}
		}
		
		flags.numTimesPruneFeatures = 0;
		for (int i = 0; i <= flags.numTimesPruneFeatures; i++) {
			logger.debug("numTimesPruneFeatures:" + i);
			timer.start();

			/**********************
			 * 添加特征选择步骤 传入的参数是featureCoordinates(一个特征出现的文档位置) 和CRFCliqueTree
			 * 
			 **********************/
			...
			{
			if (i > 1) {
				CRFFeatureSelection crfFeatureSelection = new CRFFeatureSelection(
						cliqueTrees, featureCoordinates, featureIndex,
						classIndex, map, Ehat, labelIndices, labels, flags,
						activeMap, i, selectMap);
				crfFeatureSelection.selection_bat();
			}
			}

			Pair<int[][][][], int[][]> dataAndLabels = documentsToDataAndLabels(docs);
			elapsedMs = timer.stop();
			System.err.println("Time to convert docs to data/labels: "
					+ Timing.toSecondsString(elapsedMs) + " seconds");

			Evaluator[] evaluators = null;
			if (flags.evaluateIters > 0) {
				logger.debug("flags.evaluateIters:" + i);
				List<Evaluator> evaluatorList = new ArrayList<Evaluator>();
				evaluatorList.add(new MemoryEvaluator());
				if (flags.evaluateTrain) {
					logger.debug("flags.evaluateTrain:" + i);
					...
					for (int j = 0; j < dataAndLabels.first().length; j++) {
						Pair<int[][][], int[]> p = new Pair<int[][][], int[]>(
								dataAndLabels.first()[j],
								dataAndLabels.second()[j]);
				...
				if (flags.testFile != null) {
					...
					ObjectBank<List<IN>> testObjBank = makeObjectBankFromFile(
							flags.testFile, readerAndWriter);
				...
				if (flags.testFiles != null) {
					...
					for (String testFile : testFiles) {
						...
						List<Pair<int[][][], int[]>> testDataAndLabels = documentsToDataAndLabelsList(testObjBank);
						crfEvaluator
								.setTestData(testObjBank, testDataAndLabels);
						crfEvaluator.setEvalCmd(flags.evalCmd);
						evaluatorList.add(crfEvaluator);
					}
				}
			...
			int numFeatures = featureIndex.size();
			logger.debug("numFeatures:" + numFeatures);
			...
			if (flags.loadProcessedData != null) {
				logger.debug("flags.loadProcessedData:"
			...
			if (flags.nonLinearCRF) {
				logger.debug("flags.nonLinearCRF:" + flags.nonLinearCRF);
				if (flags.secondOrderNonLinear) {
					logger.debug("flags.secondOrderNonLinear:"
							+ flags.secondOrderNonLinear);
					CRFNonLinearSecondOrderLogConditionalObjectiveFunction func = new CRFNonLinearSecondOrderLogConditionalObjectiveFunction(
							data, labels, windowSize, classIndex, labelIndices,
							map, flags, nodeFeatureIndicesMap,
							edgeFeatureIndicesMap);
					double[] allWeights = trainWeightsUsingNonLinearCRF(func);
					// System.out.println("allWeights.length:"+allWeights.length);
					Quadruple<double[][], double[][], double[][], double[][]> params = func
							.separateWeights(allWeights);
					...
					logger.debug("not flags.secondOrderNonLinear:"
							+ flags.secondOrderNonLinear);
					CRFNonLinearLogConditionalObjectiveFunction func = new CRFNonLinearLogConditionalObjectiveFunction(
							data, labels, windowSize, classIndex, labelIndices,
							map, flags, nodeFeatureIndicesMap,
							edgeFeatureIndicesMap);
					double[] allWeights = trainWeightsUsingNonLinearCRF(func);
					//System.out.println("allWeights1.length:"
					//		+ allWeights.length);
					Triple<double[][], double[][], double[][]> params = func
							.separateWeights(allWeights);
				...
				logger.debug("not flags.nonLinearCRF:" + flags.nonLinearCRF);
				double[] oneDimWeights = null;
				// System.out.println("flags.useFloat:"+flags.useFloat);
				// System.out.println("flags.numLopExpert :"+flags.numLopExpert
				// );
				if (flags.useFloat) {
					logger.debug("flags.useFloat:" + flags.useFloat);
					oneDimWeights = trainWeightsUsingFloatCRF(data, labels, i);
				} else if (flags.numLopExpert > 1) {
					logger.debug("flags.numLopExpert:" + flags.numLopExpert);

					oneDimWeights = trainWeightsUsingLopCRF(numFeatures, data,
							labels, evaluators, i);

				} else {
					// System.out.println("i :"+i );
					logger.debug("not flags.useFloat:" + flags.useFloat);
					oneDimWeights = trainWeightsUsingDoubleCRF(data, labels,
							evaluators, i);
					// System.out.println("oneDimWeights.length:"+oneDimWeights.length);
				}
				this.weights = CRFLogConditionalObjectiveFunction.to2D(
						oneDimWeights, labelIndices, map);
			}
		...
		//压缩模型，将activeMap不为1的特征去掉
		//要压缩的包括featureIndex,weights
		//to do
		if(flags.isInnerFeatureSelection==true)
		{
		  compactCRFModel();
	...
	protected double[] trainWeightsUsingFloatCRF(int[][][][] data,
			int[][] labels, int pruneFeatureItr) {
			...
			try {
				...
				DataInputStream dis = new DataInputStream(
						new BufferedInputStream(new GZIPInputStream(
								new FileInputStream(flags.initialWeights))));
		...
		float[] weights = minimizer.minimize(func, (float) flags.tolerance,
				initialWeights);
	...
	/*
	 * protected int[][] getFeatureBoundaryIndices(int numFeatures, int
	 * numLopExpert) { // first find begin/end feature index for each expert int
	 * interval = numFeatures / numLopExpert; int[] beginFeatureIndices = new
	 * int[numLopExpert]; int[] begin1DParamIndices = new int[numLopExpert];
	 * int[] endFeatureIndices = new int[numLopExpert]; int[] end1DParamIndices
	 * = new int[numLopExpert]; for (int lopIter = 0; lopIter < numLopExpert;
	 * lopIter++) { int beginIndex = lopIter * interval; int endIndex =
	 * (lopIter+1) * interval; if (lopIter == numLopExpert - 1) { endIndex =
	 * numFeatures; } int begin1D = 0; for (int fIndex = 0; fIndex < beginIndex;
	 * fIndex++) { begin1D += labelIndices[map[fIndex]].size(); } int end1D =
	 * begin1D; for (int fIndex = beginIndex; fIndex < endIndex; fIndex++) {
	 * end1D += labelIndices[map[fIndex]].size(); } beginFeatureIndices[lopIter]
	 * = beginIndex; endFeatureIndices[lopIter] = endIndex;
	 * begin1DParamIndices[lopIter] = begin1D; end1DParamIndices[lopIter] =
	 * end1D; } int[][] boundaryIndices = new int[4][]; boundaryIndices[0] =
	 * beginFeatureIndices; boundaryIndices[1] = endFeatureIndices;
	 * boundaryIndices[2] = begin1DParamIndices; boundaryIndices[3] =
	 * end1DParamIndices; return boundaryIndices; }
	 */
	...
	protected double[] trainWeightsUsingLopCRF(int numFeatures,
			int[][][][] data, int[][] labels, Evaluator[] evaluators,
			int pruneFeatureItr) {
		...
		if (flags.initialLopWeights != null) {
			try {
				...
				System.err.println("Reading initial LOP weights from file "
						+ flags.initialLopWeights + " ...");
				BufferedReader br = new BufferedReader(new InputStreamReader(
						new FileInputStream(new File(flags.initialLopWeights))));
				String line = null;
				...
				// BufferedInputStream(new GZIPInputStream(new FileInputStream(
				// flags.initialLopWeights))));
			...
			for (int lopIter = 0; lopIter < numLopExpert; lopIter++) {
				...
				if (flags.randomLopWeights) {
					lopExpertWeights[lopIter] = initWeightsUsingDoubleCRF(
							partialData, labels, evaluators, pruneFeatureItr);
				} else {
					lopExpertWeights[lopIter] = trainWeightsUsingDoubleCRF(
							partialData, labels, evaluators, pruneFeatureItr);
				}
			}
			if (flags.includeFullCRFInLOP) {
				double[][] newLopExpertWeights = new double[numLopExpert + 1][];
				System.arraycopy(lopExpertWeights, 0, newLopExpertWeights, 0,
						lopExpertWeights.length);
				if (flags.randomLopWeights) {
					newLopExpertWeights[numLopExpert] = initWeightsUsingDoubleCRF(
							data, labels, evaluators, pruneFeatureItr);
				} else {
					newLopExpertWeights[numLopExpert] = trainWeightsUsingDoubleCRF(
							data, labels, evaluators, pruneFeatureItr);
				}
		...
		CRFLogConditionalObjectiveFunctionForLOP func = new CRFLogConditionalObjectiveFunctionForLOP(
				data, labels, lopExpertWeights, windowSize, classIndex,
				labelIndices, map, flags.backgroundSymbol, numLopExpert,
				featureIndicesSetArray, featureIndicesListArray,
		...
		if (flags.initialLopScales == null) {
			...
			try {
				System.err.println("Reading initial LOP scales from file "
						+ flags.initialLopScales);
				DataInputStream dis = new DataInputStream(
						new BufferedInputStream(new GZIPInputStream(
								new FileInputStream(flags.initialLopScales))));
				initialScales = Convert.readDoubleArr(dis);
			} catch (IOException e) {
				throw new RuntimeException(
		...
		double[] learnedParams = minimizer.minimize(func, flags.tolerance,
				initialScales);
		...
		for (int lopIter = 0; lopIter < numLopExpert; lopIter++) {
			System.err.println("lopScales[" + lopIter + "] = "
					+ lopScales[lopIter]);
		}
		double[][] learnedLopExpertWeights = lopExpertWeights;
		if (flags.backpropLopTraining) {
			learnedLopExpertWeights = func
					.separateLopExpertWeights(learnedParams);
		}
		return CRFLogConditionalObjectiveFunctionForLOP
				.combineAndScaleLopWeights(numLopExpert,
						learnedLopExpertWeights, lopScales);
	}

	protected double[] initWeightsUsingDoubleCRF(int[][][][] data,
			int[][] labels, Evaluator[] evaluators, int pruneFeatureItr) {
	...
	protected double[] trainWeightsUsingNonLinearCRF(
		...
		// System.out.println("initialWeights.length:"+initialWeights);
		double[] w = minimizer.minimize(func, flags.tolerance, initialWeights);
		return w;
	}

	protected double[] trainWeightsUsingDoubleCRF(int[][][][] data,
			int[][] labels, Evaluator[] evaluators, int pruneFeatureItr) {
		logger.debug("in CRFClassifier trainWeightsUsingDoubleCRF");
		...
		if(flags.isInnerFeatureSelection==true)
		{
		cliqueTrees = func.getCRFCliqueTree();
		}
		if (flags.inputDropOut != 0.0) {
			...
			System.err.printf("Scaled weights by %f",
					1.0 / (1.0 - flags.inputDropOut));
		}
		return ws;
	}
	 ...
	 */
	protected List<CRFDatum<? extends Collection<String>, ? extends CharSequence>> extractDatumSequence(
			int[][][] allData, int beginPosition, int endPosition,
		...
		for (int position = beginContext; position < beginPosition; position++) {
			...
			CRFDatum<Collection<String>, String> datum = new CRFDatum<Collection<String>, String>(
	 ...
	 */
	protected void addProcessedData(
		...
		for (int i = 0, pdSize = processedData.size(); i < pdSize; i++) {
			...
			List<CRFDatum<Collection<String>, String>> document = processedData
					.get(i);
	...
	protected static List<List<CRFDatum<Collection<String>, String>>> loadProcessedData(
			String filename) {
		System.err.print("Loading processed data from serialized file...");
		ObjectInputStream ois = null;
		List<List<CRFDatum<Collection<String>, String>>> result = Collections
				.emptyList();
		try {
			ois = new ObjectInputStream(new FileInputStream(filename));
			result = (List<List<CRFDatum<Collection<String>, String>>>) ois
					.readObject();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void loadTextClassifier(String text, Properties props)
		...
		if (flags.nonLinearCRF) {
			...
			if (flags.secondOrderNonLinear) {
				...
				while (count < weightsLength) {
					...
					for (int i2 = 0; i2 < weights2Length; i2++) {
						inputLayerWeights4Edge[count][i2] = Double
								.parseDouble(weightsValue[i2]);
					}
					count++;
				}
				/***************************************/
				System.err.printf(
						"DEBUG: double[%d][] inputLayerWeights4Edge loaded\n",
						weightsLength);
				/***************************************/
				...
				while (count < weightsLength) {
					...
					for (int i2 = 0; i2 < weights2Length; i2++) {
						outputLayerWeights4Edge[count][i2] = Double
								.parseDouble(weightsValue[i2]);
					}
					count++;
				}
				/***************************************/
				System.err.printf(
						"DEBUG: double[%d][] outputLayerWeights loaded\n",
						weightsLength);
				/***************************************/
			...
			while (count < weightsLength) {
				...
				for (int i2 = 0; i2 < weights2Length; i2++) {
					inputLayerWeights[count][i2] = Double
							.parseDouble(weightsValue[i2]);
				}
				count++;
			}
			...
			while (count < weightsLength) {
				...
				for (int i2 = 0; i2 < weights2Length; i2++) {
					outputLayerWeights[count][i2] = Double
							.parseDouble(weightsValue[i2]);
				}
				count++;
			}
		...
		featureFactory = (edu.stanford.nlp.sequences.FeatureFactory<IN>) Class
	...
	public void serializeClassifier(String serializePath) {
		...
		try {
			...
			/*
			 * Jedis jedis = new Jedis("127.0.0.1"); jedis.select(5); String
			 * tempFeat=""; for(int i=0;i<featureIndex.size();i++) { //
			 * System.out
			 * .println("feat:"+tempFeat+" "+featureIndex.indexOf(tempFeat));
			 * tempFeat=featureIndex.get(i);
			 * jedis.set(tempFeat,featureIndex.indexOf(tempFeat)+""); }
			 */
			...
			/*
			 * Jedis jedis2 = new Jedis("127.0.0.1"); jedis2.select(6); for(int
			 * i=0;i<weights.length;i++) { for(int j=0;j<weights[i].length;j++)
			 * { jedis2.set(i+":"+j, weights[i][j]+""); } }
			 */
	...
	public void loadClassifier(ObjectInputStream ois, Properties props)
		...
		// System.out.println("labelIndices.length:"+labelIndices.length);
		classIndex = (Index<String>) ois.readObject();
		// System.out.println("classIndex.length:"+classIndex.size());
		featureIndex = (Index<String>) ois.readObject();
		/*
		 * Jedis jedis = new Jedis("127.0.0.1"); jedis.select(5); String
		 * tempFeat=""; for(int i=0;i<featureIndex.size();i++) { //
		 * System.out.println
		 * ("feat:"+tempFeat+" "+featureIndex.indexOf(tempFeat));
		 * tempFeat=featureIndex.get(i);
		 * jedis.set(tempFeat,featureIndex.indexOf(tempFeat)+""); }
		 */
		// System.out.println(" featureIndex .length:"+ featureIndex .size());
		...
		// jedis2.select(6);
		/*
		 * FileWriter fw=new FileWriter(new File("feat_file.txt")); PrintWriter
		 * pw=new PrintWriter(fw); for(int i=0;i<weights.length;i++) { for(int
		 * j=0;j<weights[i].length;j++) { // jedis2.set(i+":"+j,
		 * weights[i][j]+""); pw.println(i+":"+j+"   "+weights[i][j]+""); } }
		 * fw.close(); pw.close();
		 */
	 ...
	 */
	public static <IN extends CoreMap> CRFClassifier<IN> getJarClassifier(
	 ...
	 *             If there are problems interpreting the serialized data
	 */
	public static CRFClassifier<? extends CoreMap> getClassifier(InputStream in)
			throws IOException, ClassCastException, ClassNotFoundException {
	...
	public static CRFClassifier<CoreLabel> getClassifierNoExceptions(
			String loadPath) {
	...
	public static CRFClassifier<? extends CoreMap> getClassifier(
			String loadPath, Properties props) throws IOException,
	...
	public static void main(String[] args) throws Exception {
		...
		if (loadPath != null) {
			...
			//System.out.println("load the text classifier");
			System.err
			...
			//System.out.println("load the jar classifier");
			crf.loadJarClassifier(crf.flags.loadJarClassifier, props);
		} else if (crf.flags.trainFile != null
				|| crf.flags.trainFileList != null) {
			logger.debug("crf.flags.trainFile:" + crf.flags.trainFile);
			logger.debug("crf.flags.trainFileList:" + crf.flags.trainFileList);
			crf.train();
		} else {
			//System.out.println("load the default classifier");
			crf.loadDefaultClassifier();
		}
	...
	public List<IN> classifyWithGlobalInformation(List<IN> tokenSeq,
			final CoreMap doc, final CoreMap sent) {
	...
	public void initEmpiricalCounts(Collection<List<IN>> docs) {
		Pair<int[][][][], int[][]> dataAndLabels = documentsToDataAndLabels(docs);
		int[][][][] data = dataAndLabels.first();
		int[][] labels = dataAndLabels.second();
	...
	public void compactCRFModel()
	{
		Index<String> compactFeatureIndex = new HashIndex<String>();
		
		int activeFeatureNum=0;
		for(int i=0;i<featureIndex.size();i++)
		{
			if(activeMap[i]==1)
			{
		...
		double[][] compactWeights=new double[activeFeatureNum][];
		...
		for(int i=0;i<featureIndex.size();i++)
		{
			if(activeMap[i]==1)
			{
				compactFeatureIndex.add(featureIndex.get(i));
				compactWeights[compactFeatureIndex.indexOf(featureIndex.get(i))]=weights[i];
			}
		}
		
		
		featureIndex=compactFeatureIndex;
