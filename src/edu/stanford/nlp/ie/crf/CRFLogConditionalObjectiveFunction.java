...
public class CRFLogConditionalObjectiveFunction extends
	...
	private final double epsilon = 0.1; // You can't actually set this at
										// present
	/** label indices - for all possible label sequences - for each feature */
	private final Index<CRFLabel>[] labelIndices;
	private final Index<String> classIndex; // didn't have <String> before.
											// Added since that's what is
											// assumed everywhere.
	private final Index featureIndex; // todo [cdm]: Unused. Remove
	private final double[][] Ehat; // empirical counts of all the features
									// [feature][class]
	...
	private static Logger logger = Logger
			.getLogger(CRFLogConditionalObjectiveFunction.class);
	...
	CRFLogConditionalObjectiveFunction(int[][][][] data, int[][] labels,
			Index featureIndex, int window, Index<String> classIndex,
			Index[] labelIndices, int[] map, String backgroundSymbol) {
		this(data, labels, featureIndex, window, classIndex, labelIndices, map,
				QUADRATIC_PRIOR, backgroundSymbol);
	}
	...
	CRFLogConditionalObjectiveFunction(int[][][][] data, int[][] labels,
			...
			double sigma) {
		this(data, labels, featureIndex, window, classIndex, labelIndices, map,
				QUADRATIC_PRIOR, backgroundSymbol, sigma);
	}

	CRFLogConditionalObjectiveFunction(int[][][][] data, int[][] labels,
			Index featureIndex, int window, Index<String> classIndex,
			Index[] labelIndices, int[] map, int prior, String backgroundSymbol) {
		this(data, labels, featureIndex, window, classIndex, labelIndices, map,
				prior, backgroundSymbol, 1.0);
	}

	CRFLogConditionalObjectiveFunction(int[][][][] data, int[][] labels,
			Index featureIndex, int window, Index<String> classIndex,
			Index[] labelIndices, int[] map, int prior,
			String backgroundSymbol, double sigma) {
		...
		this.sigma = sigma;
		Ehat = empty2D();// Ehat是二维数组，行是特征的索引，列是该特征的不同标记的索引，某个特征的不同标记数和该特征属于的窗口尺寸有关
	...
	// this used to be computed lazily, but that was clearly erroneous for
	// multithreading!
	@Override
	 ...
	 * 
	 * the first element is the mapped index of featuresIndex the second element
	 * is the index of the of the element
	 * 
	 * @return a 2D weight array
	 */
	public static double[][] to2D(double[] weights, Index[] labelIndices,
			int[] map) {
		double[][] newWeights = new double[map.length][];
		int index = 0;
		for (int i = 0; i < map.length; i++) {
			newWeights[i] = new double[labelIndices[map[i]].size()];
			System.arraycopy(weights, index, newWeights[i], 0,
					labelIndices[map[i]].size());
	...
	public static double[] to1D(double[][] weights, int domainDimension) {
		...
		for (int i = 0; i < weights.length; i++) {
			System.arraycopy(weights[i], 0, newWeights, index,
					weights[i].length);
	...
	private double[][] empty2D() {
		...
		for (int i = 0; i < map.length; i++) {
			...
			// cdm july 2005: below array initialization isn't necessary: JLS
			// (3rd ed.) 4.12.5
			// Arrays.fill(d[i], 0.0);
			// index += labelIndices[map[i]].size();
		}
	...
	private void empiricalCounts(int[][][][] data, int[][] labels) {
		logger.debug(" empiricalCounts window:" + window);
		for (int m = 0; m < data.length; m++) {
			...
			if (docLabels.length > docData.length) { // only true for
														// self-training
				// fill the windowLabel array with the extra docLabels
				System.arraycopy(docLabels, 0, windowLabels, 0,
						windowLabels.length);
				// shift the docLabels array left
				int[] newDocLabels = new int[docData.length];
				System.arraycopy(docLabels, docLabels.length
						- newDocLabels.length, newDocLabels, 0,
						newDocLabels.length);
				docLabels = newDocLabels;
			}
			logger.debug(" empiricalCounts m:" + m);
			for (int i = 0; i < docData.length; i++) {
				String tstr = "";
				System.arraycopy(windowLabels, 1, windowLabels, 0, window - 1);

				for (int ti = 0; ti < window; ti++) {
					tstr += (windowLabels[ti] + " ");
				}

				windowLabels[window - 1] = docLabels[i];
				logger.debug(" empiricalCounts i:" + i + " windowLabels:"
						+ tstr + " docLabel:" + docLabels[i]);
				for (int j = 0; j < docData[i].length; j++) {
					tstr = "";
					int[] cliqueLabel = new int[j + 1];
					System.arraycopy(windowLabels, window - 1 - j, cliqueLabel,
							0, j + 1);
					for (int ti = 0; ti < cliqueLabel.length; ti++) {
						tstr += (cliqueLabel[ti] + " ");
					}
					logger.debug(" empiricalCounts j:" + j + " cliqueLabel:"
							+ tstr);
					...
					for (int k = 0; k < docData[i][j].length; k++) {
						Ehat[docData[i][j][k]][labelIndex]++;//这里是记录个数，并未正规化
					}
				}
			}
		}
		...
		/*
		 * for(int i=0;i<Ehat.length;i++) { String EStr=""; for(int
		 * j=0;j<Ehat[i].length;j++) { EStr+=(j+":"+Ehat[i][j]+" "); }
		 * logger.info("Ehat "+i+" "+EStr);
		 * 
		 * }
		 */
	}
	...
	/**
	 * Calculates both value and partial derivatives at the point x, and save
	 * them internally.
	 */
	@Override
	public void calculate(double[] x) {
		logger.debug(" calculate x[]");
		double prob = 0.0; // the log prob of the sequence given the model,
							// which is the negation of value at this point
		...
		cliqueTrees = new CRFCliqueTree[data.length];
		...
		for (int m = 0; m < data.length; m++) {
			...
			logger.debug("calculate x[] m=" + m + "  numClasses:" + numClasses);

			CRFCliqueTree cliqueTree = CRFCliqueTree.getCalibratedCliqueTree(
					weights, docData, labelIndices, numClasses, classIndex,
					backgroundSymbol);
			cliqueTrees[m] = cliqueTree;
			// compute the log probability of the document given the model with
			// the parameters x
			...
			for (int i = 0; i < docData.length; i++) {
				...
				double p = cliqueTree.condLogProbGivenPrevious(i, label, given);// 给定前一个位置的标记,当前位置标记取值label的概率
				if (VERBOSE) {
					System.err.println("P(" + label + "|"
							+ ArrayMath.toString(given) + ")=" + p);
				}
				prob += p;
			...
			// compute the expected counts for this document, which we will need
			...
			for (int i = 0; i < data[m].length; i++) {
				...
				for (int j = 0; j < data[m][i].length; j++) {
					...
					// System.out.println("labelIndex.size:"+labelIndex.size());
					// for each possible labeling for that clique
					for (int k = 0; k < labelIndex.size(); k++) {

						int[] label = labelIndex.get(k).getLabel();
						double p = cliqueTree.prob(i, label); // probability of
																// these labels
																// occurring in
																// this clique
																// with these
		...
		// compute the partial derivative for each feature by comparing expected
		// counts to empirical counts
		int index = 0;
		for (int i = 0; i < E.length; i++) {
			for (int j = 0; j < E[i].length; j++) {
				derivative[index++] = (E[i][j] - Ehat[i][j]);
				if (VERBOSE) {
					System.err.println("deriv(" + i + "," + j + ") = "
							+ E[i][j] + " - " + Ehat[i][j] + " = "
							+ derivative[index - 1]);
				}
			}
	...
	public void calculateStochasticGradientOnly(double[] x, int[] batch) {
		...
		double batchScale = ((double) batch.length)
		...
		for (int ind : batch) {
			...
			for (int i = 0; i < data[ind].length; i++) {
				...
				for (int j = 0; j < data[ind][i].length; j++) {
					...
					// System.out.println("labelIndex.size:"+labelIndex.size());
					for (int k = 0; k < labelIndex.size(); k++) {
						// CRFLabel lqlab=(CRFLabel) labelIndex.get(k);
						// System.out.println("lqlab:"+lqlab.toString());
		...
		if (prior == QUADRATIC_PRIOR) {
			...
			for (int i = 0; i < x.length; i++) {
				...
				if (wabs < epsilon) {
					...
					derivative[i] += batchScale * ((w < 0.0) ? -1.0 : 1.0)
							/ sigmaSq;
				}
			}
		} else if (prior == QUARTIC_PRIOR) {
	...
	/**
	 * Performs stochastic update of weights x (scaled by xscale) based on
	 * samples indexed by batch. NOTE: This function does not do regularization
	 * (regularization is done by the minimizer).
	 * 
	 * @param x
	 *            - unscaled weights
	 * @param xscale
	 *            - how much to scale x by when performing calculations
	 * @param batch
	...
	public double calculateStochasticUpdate(double[] x, double xscale,
		...
		for (int ind : batch) {
			...
			// make a clique tree for this document
			CRFCliqueTree cliqueTree = CRFCliqueTree.getCalibratedCliqueTree(x,
					xscale, wis, docData, labelIndices, numClasses, classIndex,
					backgroundSymbol);
			...
			for (int i = 0; i < docData.length; i++) {
				...
				for (int j = 0; j < data[ind][i].length; j++) {
					if (j > 0) {
						System.arraycopy(given, window - j - 1,
								docCliqueLabels[j], 0, j);
					}
					docCliqueLabels[j][j] = label;
					// TODO: We can eliminate this lookup by saving the
					// correctLabelIndex (or marking it in CRFLabel)
				...
				for (int j = 0; j < data[ind][i].length; j++) {
					...
					for (int k = 0; k < labelIndex.size(); k++) {
						...
						double p = cliqueTree.prob(i, label); // probability of
																...
																// features
						for (int n = 0; n < data[ind][i][j].length; n++) {
							// Adjust weight by -p*gscale (expected count
							// scaled)
	 ...
	 * only over samples indexed by batch. NOTE: This function does not do
	 * regularization (regularization is done by the minimizer).
	 * 
	 * @param x
	 *            - unscaled weights
	 * @param xscale
	...
	public double valueAt(double[] x, double xscale, int[] batch) {
		...
		double prob = 0; // the log prob of the sequence given the model, which
							// is the negation of value at this point
		int[][] wis = getWeightIndices();
	...
	public CRFCliqueTree[] getCRFCliqueTree() {
		return cliqueTrees;
	}
