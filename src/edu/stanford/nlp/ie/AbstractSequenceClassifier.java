...
public abstract class AbstractSequenceClassifier<IN extends CoreMap> implements
	...
	private static Logger logger = Logger
			.getLogger(AbstractSequenceClassifier.class);
	...
	public AbstractSequenceClassifier(SeqClassifierFlags flags) {
		...
		// try {
		this.featureFactory = new MetaClass(flags.featureFactory)
				.createInstance(flags.featureFactoryArgs);
	...
	protected final void reinit() {
		...
		if (flags.readerAndWriter != null
			...
			//System.out.println("use defaultReaderAndWriter");
			plainTextReaderAndWriter = defaultReaderAndWriter;
		} else {
			//System.out.println("not use defaultReaderAndWriter");
	...
	public DocumentReaderAndWriter<IN> makeReaderAndWriter() {
		...
		try {
			readerAndWriter = (DocumentReaderAndWriter<IN>) Class.forName(
					flags.readerAndWriter).newInstance();
		} catch (Exception e) {
	 ...
	 *         token is stored in its {@link AnswerAnnotation} field.
	 */
		...
		return new Sampler<List<IN>>() {
			...
			public List<IN> drawSample() {
				...
				for (IN word : input) {
					...
					newWord.set(AnswerAnnotation.class,
	...
	public Counter<List<IN>> classifyKBest(List<IN> doc,
		...
		for (int[] seq : bestSequences.keySet()) {
			...
			for (IN fi : doc) {
				...
				fi.remove(AnswerAnnotation.class); // because fake answers will
													// get
				...
				if (guessedAnswer.equals(flags.backgroundSymbol)) {
					...
					if (!guessedAnswer.equals(prevEntityType)) {
						...
						prevEntity = new Triple<String, Integer, Integer>(
								guessedAnswer,
								fl.get(CharacterOffsetBeginAnnotation.class),
								fl.get(CharacterOffsetEndAnnotation.class));
					} else {
						assert prevEntity != null; // if you read the code
													// carefully, this
													// should always be true!
						prevEntity.setThird(fl
	...
	public void train() {
		if (flags.trainFiles != null) {
			...
			logger.info(" train flags.trainFile:" + flags.trainFile
					+ " defaultReaderAndWriter:"
					+ defaultReaderAndWriter.getClass().getCanonicalName());
	...
	public void classifyStdin(DocumentReaderAndWriter<IN> readerWriter)
		...
		for (String line; (line = is.readLine()) != null;) {
			ObjectBank<List<IN>> documents = makeObjectBankFromString(line,
					readerWriter);
	...
	public void classifyAndWriteViterbiSearchGraph(String testFile,
		...
		for (List<IN> doc : documents) {
			...
			if (readerAndWriter instanceof LatticeWriter)
				((LatticeWriter<IN>) readerAndWriter).printLattice(tagLattice,
						doc, latticeWriter);
	...
	public static boolean countResultsIOB(List<? extends CoreMap> doc,
		...
		while (index < doc.size()) {
			index = tallyOneEntityIOB(doc, index, GoldAnswerAnnotation.class,
					AnswerAnnotation.class, entityTP, entityFN, background);
		}
		index = 0;
		while (index < doc.size()) {
			index = tallyOneEntityIOB(doc, index, AnswerAnnotation.class,
					GoldAnswerAnnotation.class, null, entityFP, background);
		}
	...
	public void loadClassifierNoExceptions(String loadPath, Properties props) {
		...
		// takes priority over the file system.
		if ((is = loadStreamFromClasspath(loadPath)) != null) {
			//System.out.println("load stream from classpath");
			Timing.startDoing("Loading classifier from " + loadPath);
			...
			//System.out.println("load stream from classpath2");
	 ...
	 * it is so stored). The classifier should be specified as its full
	 * filename, but the path in the jar file (<code>/classifiers/</code>) is
	 * coded in this class. If the classifier is not stored in the jar file or
	 * this is not run from inside a jar file, then this function will throw a
	 * RuntimeException.
	 * 
	 * @param modelName
	...
	protected void printFeatures(IN wi, Collection<String> features) {
		...
		if (cliqueWriter == null) {
			cliqueWriter = IOUtils.getPrintWriterOrDie("feats-"
					+ flags.printFeatures + ".txt");
			writtenNum = 0;
		}
		if (wi instanceof CoreLabel) {
			cliqueWriter
					.print(wi.get(TextAnnotation.class)
							+ ' '
							+ wi.get(PartOfSpeechAnnotation.class)
							+ ' '
