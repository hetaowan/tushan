...
public class ExactBestSequenceFinder implements BestSequenceFinder {
  ...
  private static int[] bestSequenceNew(SequenceModel ts) {
   ...
   // System.out.println("leftWindow:"+leftWindow);
    int rightWindow = ts.rightWindow();
   // System.out.println("rightWindow:"+rightWindow);
  ...
  //  System.out.println("padLength:"+padLength);
    ...
    //  System.out.println("tagNum["+pos+"]="+tagNum[pos]);
    ...
    {
    	//System.out.println(i+":"+productSizes[i]);
    }
    ...
    for (int pos = leftWindow; pos < leftWindow + length; pos++) {
      ...
      for (int product = 0; product < productSizes[pos]; product++) {
        ...
        if (tempTags[pos] == tags[pos][0]) {
         ...
         //   System.out.println("["+pos+"]["+(product+t*shift)+"]="+windowScore[pos][product + t * shift]);
          }
        }
      }
    }
      ...
      for (int product = 0; product < productSizes[pos]; product++) {
        ...
        if (pos == leftWindow) {
          ...
          for (int newTagNum = 0; newTagNum < tagNum[pos - leftWindow - 1]; newTagNum++) {
            ...
            double predScore = score[pos - 1][predProduct] + windowScore[pos][product];//雅可比前向迭代公式
   ...
   // System.out.println("lastProduct:"+lastProduct);
    ...
    //  System.out.println("bestCurrentProduct:"+bestCurrentProduct);
