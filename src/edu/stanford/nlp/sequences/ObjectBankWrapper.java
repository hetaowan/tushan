...
public class ObjectBankWrapper<IN extends CoreMap> extends ObjectBank<List<IN>> {
  ...
  private static Logger logger = Logger.getLogger(ObjectBankWrapper.class);
  ...
  public Iterator<List<IN>> iterator() {
	...
	// System.out.println("in objectbankwrapper iterator");
    Iterator<List<IN>> iter = new WrappedIterator(wrapped.iterator());
    //System.out.println("out objectbankwrapper iterator");
    return iter;
  }
  ...
  private class WrappedIterator extends AbstractIterator<List<IN>> {
    ...
    public boolean hasNext() {
      while ((spilloverIter == null || !spilloverIter.hasNext()) &&
             wrappedIter.hasNext()) {
    	//System.out.println("in objectbankwrapper hasnext "+wrappedIter.getClass().getCanonicalName());
    ...
    @Override
    public List<IN> next() {
    	//System.out.println("in objectbankwrapper next");
  ...
  public List<IN> processDocument(List<IN> doc) {
	//System.out.println("in objectbankwrapper processDocument");  
    ...
    if(ClassifierCommon.verbosity_level>=ClassifierCommon.DEBUG)
    {
     ...
     {
    	  CoreLabel cl=(CoreLabel)doc.get(li);
  ...
  private void doBasicStuff(List<IN> doc) {
    ...
    for (IN fl : doc) {
      ...
      if ("edu.stanford.nlp.wordseg.Sighan2005DocumentReaderAndWriter".equalsIgnoreCase(flags.readerAndWriter)) {
       ...
       // fl.set(TextAnnotation.class,intern(fix(fl.get(CharAnnotation.class))));
