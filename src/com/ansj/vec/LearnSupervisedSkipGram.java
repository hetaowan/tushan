...
public class LearnSupervisedSkipGram {
	...
	public LearnSupervisedSkipGram(Boolean isCbow, Integer layerSize, Integer window,
	...
	public LearnSupervisedSkipGram() {
		createExpTable();
	}
	...
	public void trainOneDoc(String recLine) {
		...
		skipGramdoc( sentence, labelWN,(int) nextRandom % window);
	}
     ...
     * @param neu1 
     */
    private void skipGramRaw(int index, List<WordNeuron> sentence,WordNeuron labelWN, int b) {
     ...
     * @param b
     */
    private void cbowGramRaw(int index, List<WordNeuron> sentence,WordNeuron labelWN, int b) {
        WordNeuron word = sentence.get(index);
        int a, c = 0;
	...
	private void skipGram(int index, List<WordNeuron> sentence,
		...
		for (a = b; a < window * 2 + 1 - b; a++) {
			...
			for (int i = 0; i < neurons.size(); i++) {
				...
				double p=( (1 - word.codeArr[i] - y) * alpha);
	 ...
	 * @param b
	 */
	private void skipGramdoc(List<WordNeuron> sentence, WordNeuron labelWN,
			int b) {
		int c=0;
		...
		for (int w = 0; w < sentence.size(); w++) {
			...
			List<Neuron> neurons = current_word.neurons;
			for (int i = 0; i < neurons.size(); i++) {
				HiddenNeuron out = (HiddenNeuron) neurons.get(i);
				double y=0;
				for (int j = 0; j < layerSize; j++) {
				...
				double p=( (1 - current_word.codeArr[i] - y) * alpha);
		...
		for(c=0;c<layerSize;c++)
		{
			labelWN.syn0[c]+=add[c];
		}
		
		*/
		
		
		WordNeuron next_word;
		
		
		for (int w = 0; w < sentence.size()-1; w++) {
			double[] add2=new double[layerSize];
			current_word = sentence.get(w);
			next_word = sentence.get(w+1);
			List<Neuron> neurons = current_word.neurons;
			for (int i = 0; i < neurons.size(); i++) {
				HiddenNeuron out = (HiddenNeuron) neurons.get(i);
				double y=0;
			...
			for (int i = 0; i < neurons.size(); i++) {
				...
				double p=( (1 - next_word.codeArr[i] - y) * alpha);
	...
	public void learnFile(File trainfile,File trfile, File tefile) throws IOException {
		...
		try {
			...
			fcf.convertFormatSuperviseNounText(trfile, trainformatFile);
		} catch (Exception e) {
		}
		*/
	...
	public static void main(String[] args) throws IOException {
		LearnSupervisedSkipGram learn = new LearnSupervisedSkipGram();
		...
		w2v.loadJavaModel("temp/medw2v/vector.mod");
		 ...
		 System.out.println(w2v.distance("小说"));
		 System.out.println(w2v.distance("电视剧"));
