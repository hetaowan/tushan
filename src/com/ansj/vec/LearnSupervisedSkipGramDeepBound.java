...
public class LearnSupervisedSkipGramDeepBound {
	...
	public LearnSupervisedSkipGramDeepBound(Boolean isCbow, Integer layerSize,
			Integer window, Double alpha, Double sample) {
		createExpTable();
		if (isCbow != null) {
			this.isCbow = isCbow;
		}
	...
	public LearnSupervisedSkipGramDeepBound() {
		createExpTable();
	}
	 ...
	 * @throws IOException
	 */
	private void trainModelSupvisedWordDoc(File file) throws IOException {
		...
		while ((temp = br2.readLine()) != null) {
			...
			trainOneDocSupervisedWordDoc(temp);
	...
	private void trainModelSupvisedOnlyLabel(File file) throws IOException {
		...
		while ((temp = br2.readLine()) != null) {
			...
			trainOneDocSupervisedOnlyLabel(temp);
	 ...
	 * @throws IOException
	 */
	private void trainModelSupvisedDoc(File file) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void trainModelUnsupervised(File file) throws IOException {
	...
	private void trainModelUnsupervisedMp(File file) throws IOException {
		...
		try (BufferedReader br = new BufferedReader(new InputStreamReader(
			...
			while ((temp = br.readLine()) != null) {
				...
				trainOneDocUnsupervisedMp(temp);
	 ...
	 * @param recLine
	 */
	public void trainOneDocSupervisedWordDoc(String recLine) {
		String label_text = "";
		String seg_text = "";
		String[] seg_arr = null;
		...
		for (int index = 0; index < sentence.size(); index++) {
			nextRandom = nextRandom * 25214903917L + 11;
			/*
			 * if (isCbow) { cbowGram(index, sentence, labelWN, (int) nextRandom
			 * % window); } else { skipGram(index, sentence, labelWN, (int)
			 * nextRandom % window); }
			 */
			// skipGramRaw(index, sentence, labelWN, (int) nextRandom % window);
			skipGram(index, sentence, labelWN, (int) nextRandom % window);
		}

		// cbowGramdoc( sentence, labelWN,(int) nextRandom % window);
		skipGramdocOneSupervisedWordDoc(sentence, labelWN, (int) nextRandom
				% window);
	}

	
	public void trainOneDocSupervisedOnlyLabel(String recLine) {
		String label_text = "";
		String seg_text = "";
		String[] seg_arr = null;
		seg_arr = recLine.split("\001");
	 ...
	 * @param recLine
	 */
	public void trainOneDocUnsupervised(String recLine) {
		String label_text = "";
		String seg_text = "";
		String[] seg_arr = null;
		seg_arr = recLine.split("\001");
		...
		for (int index = 0; index < sentence.size(); index++) {
			 ...
			 */
			// skipGramRaw(index, sentence, labelWN, (int) nextRandom % window);
			skipGramRaw(index, sentence, (int) nextRandom % window);
		}

		// cbowGramdoc( sentence, labelWN,(int) nextRandom % window);
		// skipGramdocOne(sentence, labelWN, (int) nextRandom % window);
	}

	public void trainOneDocUnsupervisedMp(String recLine) {
		String label_text = "";
		String seg_text = "";
		String[] seg_arr = null;
		seg_arr = recLine.split("\001");
		...
		for (int index = 0; index < sentence.size(); index++) {
			 ...
			 */
			// skipGramRaw(index, sentence, labelWN, (int) nextRandom % window);
			skipGramMp(index, sentence,  (int) nextRandom % window);
		}
	 ...
	 * @param recLine
	 */
	public void trainOneDocSupervisedDoc(String recLine) {
		String label_text = "";
		String seg_text = "";
		String[] seg_arr = null;
		seg_arr = recLine.split("\001");
		...
		// cbowGramdoc( sentence, labelWN,(int) nextRandom % window);
		skipGramdocSupervisedDoc(sentence, labelWN, (int) nextRandom % window);
	}
	 ...
	 * @param neu1
	 */
	private void skipGramRaw(int index, List<WordNeuron> sentence, int b) {
	 ...
	 * @param b
	 */
	private void cbowGramRaw(int index, List<WordNeuron> sentence,
	...
	private void skipGram(int index, List<WordNeuron> sentence,
		...
		if (isF == true) {
			// labelWN.syn0 = ArrayUtil.initialRandom(layerSize);
			List<Neuron> neuronsinit = labelWN.neurons;
			for (int i = 0; i < neuronsinit.size(); i++) {
				HiddenNeuron outinit = (HiddenNeuron) neuronsinit.get(i);
				outinit.syn1 = ArrayUtil.initialRandom(layerSize);
			}
		}
		...
		for (a = b; a < window * 2 + 1 - b; a++) {
			...
			for (int i = 0; i < neurons.size(); i++) {
				...
				for (c = 0; c < layerSize; c++) {
					out.syn1[c] += g * we.syn0[c];
					// out.syn1[c] += p *labelWN.syn0[c];
				}
			}

			neurons = labelWN.neurons;
			double[] nadd = new double[layerSize];
			for (int i = 0; i < neurons.size(); i++) {
				HiddenNeuron out = (HiddenNeuron) neurons.get(i);

				double y = 0;
				for (int j = 0; j < layerSize; j++) {
					y += we.syn0[j] * out.syn1[j];
				}

				if (y <= -MAX_EXP || y >= MAX_EXP) {
					continue;
				} else {
					y = (y + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2);
					y = expTable[(int) y];
				}

				double p = ((1 - labelWN.codeArr[i] - y) * alpha);
				for (int j = 0; j < layerSize; j++) {
					nadd[j] += p * out.syn1[j];
				}
	...
	private void skipGramMp(int index, List<WordNeuron> sentence,int b) {
		// TODO Auto-generated method stub
		WordNeuron word = sentence.get(index);
		if(word.name.equals("NA"))
		{
	...
	private void cbowGramUn(int index, List<WordNeuron> sentence,
			WordNeuron labelWN, int b) {
		// alpha=0.025;
		WordNeuron word = sentence.get(index);
		int a, c = 0;
	 ...
	 * @param b
	 */
	private void skipGramdocSupervisedDoc(List<WordNeuron> sentence,
			WordNeuron labelWN, int b) {
		int c = 0;
		...
		for (int w = 0; w < sentence.size(); w++) {
			...
			List<Neuron> neurons = labelWN.neurons;
			double[] add = new double[layerSize];
			for (int i = 0; i < neurons.size(); i++) {
				HiddenNeuron out = (HiddenNeuron) neurons.get(i);

				double y = 0;
				for (int j = 0; j < layerSize; j++) {
					y += current_word.syn0[j] * out.syn1[j];
				}
				...
				for (c = 0; c < layerSize; c++) {
					out.syn1[c] += p * current_word.syn0[c];
					add[c] += p * out.syn1[c];
				}
			}

			for (c = 0; c < layerSize; c++) {
				current_word.syn0[c] += add[c];
			}
	 ...
	 * @param b
	 */
	private void skipGramdocOneSupervisedWordDoc(List<WordNeuron> sentence,
			WordNeuron labelWN, int b) {
		int c = 0;

		WordNeuron current_word;
		/*
		 * boolean isF = false; if (!(occHash.containsKey(labelWN.name))) { isF
		 * = true; occHash.put(labelWN.name, "1"); } if (isF == true) {
		 * //labelWN.syn0 = ArrayUtil.initialRandom(layerSize); List<Neuron>
		 * neuronsinit =labelWN.neurons; for (int i = 0; i < neuronsinit.size();
		 * i++) { HiddenNeuron outinit = (HiddenNeuron) neuronsinit.get(i);
		 * outinit.syn1= ArrayUtil.initialRandom(layerSize); } }
		 */
		for (int w = 0; w < sentence.size(); w++) {
			current_word = sentence.get(w);
	 ...
	 * @throws IOException
	 */
	private void readWordCount(File file) throws IOException {
		trainWordsCount = 0;
		String seg_text = "";
		String label_text = "";
		String[] seg_arr = null;
	 ...
	 * @throws IOException
	 */
	public void learnFile(File trainfile, File trfile, File tefile)
			throws IOException {

		File trteformatFile = new File("temp/medw2v/trteformat.txt");
		try {
			//FileConvertFormat fcf = new FileConvertFormat();
			// fcf.readcatewords("temp/crp_output");
			//fcf.convertFormatSuperviseTrTe(trfile, tefile, trteformatFile);
		} catch (Exception e) {
		}
		readVocab(trteformatFile);
		...
		try {
			//FileConvertFormat fcf = new FileConvertFormat();
			// fcf.readcatewords("temp/crp_output");
			//fcf.convertFormatSupervise(trfile, trformatFile);
		} catch (Exception e) {
		}
		...
		try {
			//FileConvertFormat fcf = new FileConvertFormat();
			// fcf.readcatewords("temp/crp_output");
			//fcf.convertFormatSupervise(tefile, teformatFile);
		} catch (Exception e) {
		}
		
		try{
		//FileSplit.splitRandomSim(0.7,trteformatFile.getAbsolutePath(),trformatFile.getAbsolutePath(),teformatFile.getAbsolutePath());
		}
		catch(Exception e)
		{
		...
		// readWordCount(teformatFile);
		// readWordCount(trformatFile);
		int trainWordsCount1 = trainWordsCount;
		readWordCount(trformatFile);
		int trainWordsCount2 = trainWordsCount;

		for (int i = 0; i < 3; i++) {
			trainWordsCount = trainWordsCount1;
			trainModelUnsupervised(trteformatFile);
			trainWordsCount = trainWordsCount2;
			trainModelSupvisedOnlyLabel(trformatFile);
			// trainModelSupvisedDoc(trformatFile);
			////trainModelSupvisedWordDoc(trformatFile);
			trainModelSupvisedDoc(trformatFile);
			trainWordsCount = trainWordsCount1;
			trainModelUnsupervisedMp(trteformatFile);
		...
		/*
		 * LABEL[] label_set = new LABEL[labelIndex.size()]; int lin = 0;
		 * StrToClass sc = new StrToClass(); for (Integer tl :
		 * labelIndex.values()) { label_set[lin++] =
		 * sc.str2labelSimple(tl.intValue() + "", labelIndex.size()); }
		 * 
		 * PerceptronLearnSupervised pls = new PerceptronLearnSupervised(); try
		 * { pls.learnModel(trvecFile, label_set, layerSize, tevecFile); } catch
		 * (Exception e) {
		 * 
		 * }
		 */
	...
	public static void main(String[] args) throws IOException {
		LearnSupervisedSkipGramDeepBound learn = new LearnSupervisedSkipGramDeepBound();
		long start = System.currentTimeMillis();
       
		learn.learnFile(new File("temp/medw2v/train.txt"), new File(
				"temp/medw2v/tr.txt"), new File("temp/medw2v/te.txt"));
		...
		//w2v.writeModel("temp/medw2v/model.txt");
		System.out.println(w2v.distance("糟糕"));
		System.out.println(w2v.distance("漂亮"));
		System.out.println(w2v.distance("不错"));
        /*
