...
public class PlainBeamDecoderNoSearch extends BeamDecoder{
	...
	public FEATSET beam_search(WORD[] words, FEAT[] sub_weights,
		...
		FEATSET best_feat=tl.getBestFromList(src);
		worst_feat=tl.worst_feat;
		return best_feat;
	}
	
	public FEATSET beam_search_nocut(WORD[] words, double[] sub_weights,
			LABEL[] label_set, LEARNPARM learn_parms) {
		ArrayList<FEATSET> src = null;
		src = new ArrayList<FEATSET>();
		FEATSET ifs = null;// 初始feat set
		...
		for (int i = 0; i < label_set.length; i++) {
			// System.out.println("word[0]="+words[0].index);
			ifs = new FEATSET(words[0], label_set[i], new WEI(sub_weights[FeatureIndex.featIndexNoWordMap(1, label_set[i])]));
			for(int j=1;j<words.length;j++)
			{
				ifs.addWord(words[j], new WEI(sub_weights[FeatureIndex.featIndexNoWordMap(j+1, ifs.label)]));
			}	
