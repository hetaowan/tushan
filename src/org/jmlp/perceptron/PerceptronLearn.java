...
public class PerceptronLearn {
	...
	public void learnModel(File input_file, int label_num, int word_num,File test_file)
			throws Exception {
		String[] sample = null;
		...
		for (int i = 1; i <= learn_parms.label_num; i++) {
			local_label_set[i - 1] = sc.str2label(i + "", learn_parms);
		}
	...
	public void learnModelthree(File input_file, int label_num, int word_num,File test_file,LABEL[] local_label_set,Medw2vParms mw2vparms)
			throws Exception {
		//String[] sample = null;
		//sample = FileToArray.fileToDimArr(input_file);
                FileReader fr=null;
		...
		learn_parms.bd_type = "plnbdns";
		learn_parms.label_num = label_num;
		weights = new double[(word_num + 1) * (mw2vparms.first_label_num+mw2vparms.second_label_num+mw2vparms.third_label_num)];
		for (int i = 0; i < weights.length; i++) {
			weights[i] = 0;
		}
                int label_width=mw2vparms.first_label_num+mw2vparms.second_label_num+mw2vparms.third_label_num;
		int correct_num = 0;
		int incorrect_num = 0;
		...
		for (int loop = 0; loop < 2; loop++) {
			...
			//for (int ll = 0; ll < sample.length; ll++) {
				...
				FEATSETTHREE best_fset = bd.beam_search_three(words,
						wc.all2partthree(weights, words, local_label_set,label_width),
						local_label_set, learn_parms);
                               
				worst_label[Integer.parseInt(did)]=bd.sub_worst_feat.label;
				LABEL yt = sc.str2labelthree(label, mw2vparms);
				did = did.trim();
                                System.out.println("best_label:["+best_fset.label.toStr()+"] yt:["+yt.toStr()+"]");
				if (yt.equals_three(best_fset.label)) {
					...
					// weights=WeightsUpdate.regular_weight(weights);
					// PrintUtil.printNoZero(weights);
					weights = wu.update_weight_three(weights, words, best_fset.label,yt);
					incorrect_num++;
					// weights = WeightsUpdate.regular_weight_abs(weights);
					// PrintUtil.printNoZero(weights);
	 ...
	 * @return
	 */
	public  double predict_accurate(double[] weights,String[] test_sample,LABEL[] label_set,LEARNPARM learn_parms)
	{	
		double acc=0;
		try {
		String lds = "";// 一行样本
	 ...
	 * @return
	 */
	public  double predict_accurate_three(double[] weights,String[] test_sample,LABEL[] label_set,LEARNPARM learn_parms,int label_width)
	{	
		double acc=0;
		try {
		String lds = "";// 一行样本
		...
		for(int i=0;i<test_sample.length;i++){
			...
			ShuffleArray.shuffle(words);
			//best_fset = bd.beam_search(words, wc.all2part(weights, words, label_set), label_set,learn_parms);
			
			FEATSETTHREE best_fset = bd.beam_search_three(words,wc.all2partthree(weights, words, label_set,label_width),label_set, learn_parms);
			/*
			best_fset = beam_search(words, WeightsCut.all2part(
					part_weights, words, local_label_set), local_label_set,
					local_learn_parm);
