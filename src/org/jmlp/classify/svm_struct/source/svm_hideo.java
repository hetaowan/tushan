...
public class svm_hideo {
	...
	public double calculate_qp_objective(int opt_n, double[] opt_g,
		...
		for (i = 0; i < opt_n; i++) {
			obj = WeightsUpdate
					.sum(obj, WeightsUpdate.mul(opt_g0[i], alpha[i]));
			obj = WeightsUpdate.sum(
					obj,
					WeightsUpdate.mul(
							0.5,
							WeightsUpdate.mul(
									alpha[i],
									WeightsUpdate.mul(alpha[i], opt_g[i * opt_n
											+ i]))));
			for (j = 0; j < i; j++) {
				obj = WeightsUpdate.sum(obj, WeightsUpdate.mul(alpha[j],
						WeightsUpdate.mul(alpha[i], opt_g[j * opt_n + i])));
			}
		}
		return (obj);
