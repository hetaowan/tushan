...
public class svm_struct_learn {
	...
	public void svm_learn_struct_joint(SAMPLE sample, STRUCT_LEARN_PARM sparm,
		...
		// logger.info("svm_learn_struct_joint call svm_learn_optimization out loop");
	 ...
	 */
	{
		...
		if (sparm.loss_type == SLACK_RESCALING) {
			...
			ybar = svm_struct_api.find_most_violated_constraint_slackrescaling(
					ex.x, ex.y, sm, sparm);
		} else {
			// logger.info("call method find_most_violated_constraint_marginrescaling");
			ybar = svm_struct_api
					.find_most_violated_constraint_marginrescaling(ex.x, ex.y,
							sm, sparm);
		}
		if (svm_struct_common.struct_verbosity >= 2)
		...
		//rhs_i_g = lossval / n;
		//rhs_i_g = lossval / n;
		rhs_i_g=WeightsUpdate.div(lossval, n, 20);
