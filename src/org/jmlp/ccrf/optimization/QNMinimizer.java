...
public class QNMinimizer implements Minimizer<DiffFunction>, HasEvaluators {
  ...
  private static Logger logger = Logger.getLogger(QNMinimizer.class);
  private boolean isFeatureSelection=false;
  ...
  //参见 Numerical Optimization P178 Algorithm 7.4
  ...
  public double[] minimize(DiffFunction dfunction, double functionTolerance,
    ...
    do {
      ...
      try {
        ...
        if (doEval) {
        logger.debug("doEval:"+doEval);
          doEvaluation(x);
        }
        its += 1;
        double newValue;
        double[] newPoint = new double[3]; // initialized in loop
        logger.debug("Iter " + its + " evals " + fevals + " ");
  ...
  private double[] lineSearchMinPack(DiffFunction dfunc, double[] dir,
	...
	logger.debug("dfunc:"+dfunc.getClass().getCanonicalName());
    double xtrapf = 4.0;
    int info = 0;
    int infoc = 1;
    bracketed = false;
...
public void setisFeatureSelection(boolean isFeatureSelection) {
